...
export const ArticleCreator: React.FC<ArticleCreatorProps> = ({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      console.log('🖼️ Image file prepared for upload')
    }
  }
  ...
  const handleSave = async (status: 'draft' | 'published') => {
    ...
    const logStep = (step: string, data?: any) => {
      ...
      console.log(`⏱️ ${elapsed}ms - STEP ${step}`, data || '')
    }
    ...
    try {
      ...
      logStep('1: Starting handleSave', { status, contentLength: content.length })
      
      // Quick connection test before proceeding
      logStep('2: Testing connection...')
      if (!navigator.onLine) {
        throw new Error('No internet connection. Please check your connection and try again.')
      }
      logStep('3: Connection OK')

      // Auto-generate excerpt if not provided
      logStep('4: Starting excerpt generation', { hasExcerpt: !!excerpt.trim() })
      const finalExcerpt = excerpt.trim() || ArticleService.generateExcerpt(content, 200)
      logStep('5: Excerpt generated', { excerptLength: finalExcerpt.length })

      // Parse tags
      logStep('6: Parsing tags', { rawTags: tags })
      const tagArray = tags.split(',').map(tag => tag.trim()).filter(tag => tag.length > 0)
      logStep('7: Tags parsed', { tagCount: tagArray.length })

      logStep('8: Uploading image if present')
      let uploadedImageUrl = ''
      if (imageFile) {
        try {
          console.log('🖼️ Uploading image to Supabase Storage...')
          uploadedImageUrl = await ImageService.uploadImage(imageFile, 'articles')
      ...
      logStep('9: Building article data object')
      const articleData = {
        title: title.trim(),
        slug: '', // Will be auto-generated by database trigger
        content: content.trim(),
        excerpt: finalExcerpt,
        featured_image: uploadedImageUrl || featuredImage.trim() || undefined,
      ...
      logStep('10: Article data built', { 
        dataSize: JSON.stringify(articleData).length,
        hasImage: !!articleData.featured_image,
        hasYouTube: !!articleData.youtube_video_id,
        tagCount: articleData.tags?.length || 0
      })

      // ArticleService now handles timeouts internally
      logStep('11: Calling ArticleService.createArticle...')
      const article = await ArticleService.createArticle(articleData)
      logStep('12: ArticleService.createArticle completed', { articleId: article.id })
      
      // Reset retry count on success
      setRetryCount(0)
      logStep('13: Calling onSuccess...')
      onSuccess(article as any)
      logStep('14: Calling handleClose...')
      handleClose()
      logStep('15: handleSave completed successfully')
    } catch (err) {
      const elapsed = Date.now() - startTime
      console.error(`💥 ${elapsed}ms - ERROR in handleSave:`, err)
