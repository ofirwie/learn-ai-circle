...
export const ArticleCreator: React.FC<ArticleCreatorProps> = ({
  ...
  const handleSave = async (status: 'draft' | 'published') => {
    ...
    try {
      ...
      logStep('8: Building article data object')
      const articleData = {
        title: title.trim(),
      ...
      logStep('10: Calling ArticleService.createArticle...')
      const article = await ArticleService.createArticle(articleData)
      logStep('11: ArticleService.createArticle completed', { articleId: article.id })
      
      // Reset retry count on success
      setRetryCount(0)
      logStep('12: Calling onSuccess...')
      onSuccess(article as any)
      logStep('13: Calling handleClose...')
      handleClose()
      logStep('14: handleSave completed successfully')
    } catch (err) {
      const elapsed = Date.now() - startTime
